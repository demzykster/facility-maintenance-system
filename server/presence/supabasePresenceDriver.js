import { presenceRecordFromSupabaseRow, presenceRecordToSupabaseRow } from "../../src/presenceRecordModel.js";
...
export function createSupabasePresenceDriver({ url, serviceRoleKey, table = "technician_presence", fetchImpl = globalThis.fetch } = {}) {
  ...
  return {
    async list({ limit = 1000 } = {}) {
      ...
      const response = await fetchImpl(`${base}?select=*&order=last_seen_at.desc&limit=${safeLimit}`, {
        method: "GET",
        headers: serviceHeaders(serviceRoleKey)
      });
      const data = await readJsonOrText(response);
      if (!response.ok) throw new Error(errorMessage(data, `supabase_${table}_${response.status}`));
      return Array.isArray(data) ? data.map(presenceRecordFromSupabaseRow) : [];
    },
    async get(id) {
      const recordId = String(id || "").trim();
      if (!recordId) throw new Error("presence_id_required");
      ...
      const data = await readJsonOrText(response);
      if (!response.ok) throw new Error(errorMessage(data, `supabase_${table}_${response.status}`));
      return Array.isArray(data) && data[0] ? presenceRecordFromSupabaseRow(data[0]) : null;
    },
    async upsert(record) {
      const row = presenceRecordToSupabaseRow(record);
...
export function createSupabasePresenceDriverFromEnv(env = process.env, fetchImpl = globalThis.fetch) {
  return createSupabasePresenceDriver({
    url: env.SUPABASE_URL,
    serviceRoleKey: env.SUPABASE_SERVICE_ROLE_KEY,
    table: env.CMMS_PRESENCE_SUPABASE_TABLE || "technician_presence",
    fetchImpl
  });
}
