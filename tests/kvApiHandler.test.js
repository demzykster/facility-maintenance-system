...
describe("kv API handler", () => {
  ...
  it("no-ops retired production API KV mirror writes without touching storage", async () => {
    ...
    const configPutRes = await call(handler, {
      method: "PUT",
      headers: { authorization: "Bearer user-token" },
      query: { key: "config:v1", shared: "1" },
      body: { value: "{\"companyName\":\"CDSL\"}", shared: true }
    });
    const ticketPutRes = await call(handler, {
      method: "PUT",
      headers: { authorization: "Bearer user-token" },
    ...
    const userPutRes = await call(handler, {
      ...
      headers: { authorization: "Bearer user-token" },
      query: { key: "user:worker-1", shared: "1" },
      body: { value: "{\"id\":\"worker-1\"}", shared: true }
    });
    const zonePutRes = await call(handler, {
      method: "PUT",
      headers: { authorization: "Bearer user-token" },
      query: { key: "czone:zone-1", shared: "1" },
      body: { value: "{\"id\":\"zone-1\"}", shared: true }
    });
    const fleetPutRes = await call(handler, {
      method: "PUT",
      headers: { authorization: "Bearer user-token" },
      query: { key: "fleet:F-1", shared: "1" },
      body: { value: "{\"id\":\"F-1\"}", shared: true }
    });
    const pmPutRes = await call(handler, {
      method: "PUT",
      headers: { authorization: "Bearer user-token" },
    ...
    const locationPutRes = await call(handler, {
      method: "PUT",
      headers: { authorization: "Bearer user-token" },
      query: { key: "location:loc-1", shared: "1" },
      body: { value: "{\"id\":\"loc-1\"}", shared: true }
    });
    const appIssuePutRes = await call(handler, {
      method: "PUT",
      headers: { authorization: "Bearer user-token" },
      query: { key: "appIssue:issue-1", shared: "1" },
      body: { value: "{\"id\":\"issue-1\"}", shared: true }
    });
    ...
    const postRes = await call(handler, {
      ...
      body: {
        ...
        records: [
          ...
          { key: "location:loc-1", value: "{\"id\":\"loc-1\"}" },
          { key: "czone:zone-1", value: "{\"id\":\"zone-1\"}" },
          { key: "cround:round-1", value: "{\"id\":\"round-1\"}" },
          { key: "appIssue:issue-1", value: "{\"id\":\"issue-1\"}" },
    ...
    expect(configPutRes.statusCode).toBe(200);
    ...
    expect(zonePutRes.json()).toEqual({ ok: true, retired: true, retiredPrefix: "czone:" });
    expect(fleetPutRes.statusCode).toBe(200);
    ...
    expect(locationPutRes.statusCode).toBe(200);
    expect(locationPutRes.json()).toEqual({ ok: true, retired: true, retiredPrefix: "location:" });
    expect(appIssuePutRes.statusCode).toBe(200);
    expect(appIssuePutRes.json()).toEqual({ ok: true, retired: true, retiredPrefix: "appIssue:" });
