  ...
  try {
    ...
    return value
      .map((item) => ({
        ...
        userGroups: Array.isArray(item.userGroups) ? item.userGroups : [],
...
export function upsertPushSubscription(list = [], subscription = {}, user = {}, now = Date.now()) {
  ...
  const record = {
    ...
    userGroups: Array.isArray(user.userGroups) ? user.userGroups : (Array.isArray(user.groups) ? user.groups : []),
