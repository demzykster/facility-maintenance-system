...
function UserApp(p) {
  ...
  const activeView = view === "activity" && !mayViewAudit ? "tickets" : view;
  const pageTitle = activeView === "activity" ? "יומן פעילות" : activeView === "teamAdmin" ? "צוות ומשתמשים" : activeView === "dept" ? "המחלקה שלי" : "הקריאות שלי";
  return (
    <div className="app-root">
      ...
      <div className="main-col">
        ...
        <div className="content with-nav">
          ...
          </>) : activeView === "activity" ? (<AuditLog session={session} tickets={tickets} fleet={fleet} config={config} onOpenTicket={openTicket} />) : activeView === "tasks" ? (<ManageHub {...p} />) : activeView === "teamAdmin" && mayViewUsers ? (<SettingsPanel {...p} only="users" canManageUsers={mayManageUsers} />) : (<>
...
function Analytics({ tickets: allTickets, fleet, pm, config, onFilter, ctx, setCtx }) {
  ...
  return (<>
    ...
    {!hasInsights ? <div className="note">אין תובנות לתקופה שנבחרה — נסו טווח רחב יותר.</div> : <>
      {recurList.length > 0 && <div className="panel"><div className="ins-h"><AlertTriangle size={14} color="#DC2626" /> אקטיבים עם תקלות חוזרות ({REPEAT_MIN}+ בתקופה)</div>{recurList.map((r, i) => <div key={i} className="ins-row"><span className="ins-name">{r.name}</span><span className="ins-val" style={{ color: "#DC2626" }}>{countLabel(r.n, "קריאה", "קריאות")}</span></div>)}</div>}
