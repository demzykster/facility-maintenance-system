...
const assertOk = async (response) => {
  ...
  throw new Error(body?.error || `cleaning_rounds_api_${response.status}`);
};

export function createApiCleaningRoundsProvider({ baseUrl, fetchImpl = globalThis.fetch, getAccessToken = null } = {}) {
