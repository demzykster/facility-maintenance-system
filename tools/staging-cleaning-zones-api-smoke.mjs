...
async function zoneRows({ root, serviceRoleKey, id }) {
  const response = await fetch(`${root}/rest/v1/cleaning_zones?id=eq.${encodeURIComponent(id)}&select=id,name,building,source_kv_key`, {
    headers: {
...
const id = `smoke-cleaning-zone-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
const name = `Smoke Cleaning Zone ${Date.now()}`;
const zone = {
  id,
  name,
  building: "staging",
  ...
  areaName: "staging",
  active: true,
  checklist: [{ id: "floor", label: "Floor" }],
...
try {
  ...
  const afterUpsert = await zoneRows({ root: supabaseUrl, serviceRoleKey, id });
  ...
  if (!deleteResponse.ok || !deleteData?.ok) throw new Error(deleteData?.error || `cleaning_zone_delete_${deleteResponse.status}`);

  const afterDelete = await zoneRows({ root: supabaseUrl, serviceRoleKey, id });
  if (afterDelete.length !== 0) throw new Error(`cleaning_zone_delete_row_count:${afterDelete.length}`);
