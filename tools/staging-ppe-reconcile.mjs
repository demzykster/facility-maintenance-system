...
const RESOURCES = Object.freeze([
  { resource: "movements", singular: "movement", prefix: "ppe:", table: "ppe_movements", label: "ppeMovements" },
  { resource: "items", singular: "item", prefix: "ppeitem:", table: "ppe_items", label: "ppeItems" },
  { resource: "norms", singular: "norm", prefix: "ppenorm:", table: "ppe_norms", label: "ppeNorms" },
  { resource: "requests", singular: "request", prefix: "ppereq:", table: "ppe_requests", label: "ppeRequests" },
  { resource: "orders", singular: "order", prefix: "ppeorder:", table: "ppe_orders", label: "ppeOrders" }
]);
...
async function upsertPpe({ publicUrl, accessToken, resource, singular, record }) {
  ...
  const data = await readJson(response);
  if (!response.ok || !data?.ok) throw new Error(data?.error || `ppe_${resource}_upsert_${response.status}`);
}
...
for (const config of RESOURCES) {
  ...
  for (const record of toBackfill) {
    await upsertPpe({ publicUrl, accessToken, resource: config.resource, singular: config.singular, record });
  }
  ...
  if (missing.length || mismatched.length) {
    throw new Error(`ppe_${config.resource}_reconcile_failed:missing=${missing.join(",") || "-"};mismatched=${mismatched.join(",") || "-"}`);
  }
