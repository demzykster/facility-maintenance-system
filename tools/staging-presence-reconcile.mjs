...
async function kvPresence({ publicUrl, accessToken }) {
  const response = await fetch(`${publicUrl}/api/kv?prefix=presence%3A&shared=1&includeValues=1`, {
    headers: { authorization: `Bearer ${accessToken}` }
  });
  const data = await readJson(response);
  if (!response.ok) throw new Error(data?.error || `kv_presence_${response.status}`);
  return parseKvRecords(data?.records || []);
}

async function normalizedRows({ root, serviceRoleKey }) {
  const response = await fetch(`${root}/rest/v1/technician_presence?select=id,source_kv_key,legacy_payload`, {
    headers: {
...
async function upsertPresence({ publicUrl, accessToken, presence }) {
...
const kv = await kvPresence({ publicUrl, accessToken });
...
for (const record of toBackfill) {
  await upsertPresence({ publicUrl, accessToken, presence: record });
}
...
if (missing.length || mismatched.length) {
  throw new Error(`presence_reconcile_failed:missing=${missing.join(",") || "-"};mismatched=${mismatched.join(",") || "-"}`);
}

console.log(JSON.stringify({
  ok: true,
  appUrl: publicUrl,
  presence: { kv: kv.length, normalized: afterRows.length, backfilled: toBackfill.length }
}, null, 2));
