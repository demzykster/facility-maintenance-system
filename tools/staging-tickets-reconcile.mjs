...
function parseKvTickets(records = []) {
  return records.map((record) => {
    try {
      const ticket = JSON.parse(record.value);
...
async function kvTickets({ publicUrl, accessToken }) {
  const response = await fetch(`${publicUrl}/api/kv?prefix=ticket%3A&shared=1&includeValues=1`, {
    headers: { authorization: `Bearer ${accessToken}` }
  });
  const data = await readJson(response);
  if (!response.ok) throw new Error(data?.error || `kv_tickets_${response.status}`);
  return parseKvTickets(data?.records || []);
}

async function normalizedTicketRows({ root, serviceRoleKey }) {
  const response = await fetch(`${root}/rest/v1/tickets?select=id,source_kv_key,legacy_payload`, {
    headers: {
...
async function upsertTicket({ publicUrl, accessToken, ticket }) {
  const response = await fetch(`${publicUrl}/api/tickets`, {
    method: "POST",
    headers: {
  ...
  const data = await readJson(response);
  if (!response.ok || !data?.ok) throw new Error(data?.error || `tickets_upsert_${response.status}`);
}
...
const kv = await kvTickets({ publicUrl, accessToken });
const beforeRows = await normalizedTicketRows({ root: supabaseUrl, serviceRoleKey });
const beforeById = new Map(beforeRows.map((row) => [row.id, row]));
const toBackfill = kv.filter((ticket) => {
  const row = beforeById.get(ticket.id);
  return !row || stableStringify(row.legacy_payload || {}) !== stableStringify(ticket);
});

for (const ticket of toBackfill) {
  await upsertTicket({ publicUrl, accessToken, ticket });
}

const afterRows = await normalizedTicketRows({ root: supabaseUrl, serviceRoleKey });
const afterById = new Map(afterRows.map((row) => [row.id, row]));
const missing = kv.filter((ticket) => !afterById.has(ticket.id)).map((ticket) => ticket.id);
const mismatched = kv.filter((ticket) => {
  const row = afterById.get(ticket.id);
  return row && stableStringify(row.legacy_payload || {}) !== stableStringify(ticket);
}).map((ticket) => ticket.id);

if (missing.length || mismatched.length) {
  throw new Error(`tickets_reconcile_failed:missing=${missing.join(",") || "-"};mismatched=${mismatched.join(",") || "-"}`);
}
...
console.log(JSON.stringify({
  ...
  appUrl: publicUrl,
  kvTickets: kv.length,
  normalizedTickets: afterRows.length,
