...
const RESOURCES = Object.freeze([
  { resource: "tasks", singular: "task", prefix: "mtask:", table: "maintenance_tasks", label: "tasks" },
  { resource: "meetings", singular: "meeting", prefix: "mmeet:", table: "maintenance_meetings", label: "meetings" }
]);
...
async function upsertWorkRecord({ publicUrl, accessToken, resource, singular, record }) {
  ...
  const data = await readJson(response);
  if (!response.ok || !data?.ok) throw new Error(data?.error || `work_${resource}_upsert_${response.status}`);
}
...
for (const config of RESOURCES) {
  ...
  for (const record of toBackfill) {
    await upsertWorkRecord({ publicUrl, accessToken, resource: config.resource, singular: config.singular, record });
  }
  ...
  if (missing.length || mismatched.length) {
    throw new Error(`work_${config.resource}_reconcile_failed:missing=${missing.join(",") || "-"};mismatched=${mismatched.join(",") || "-"}`);
  }
