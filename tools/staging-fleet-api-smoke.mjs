...
async function fleetRows({ root, serviceRoleKey, id }) {
  const response = await fetch(`${root}/rest/v1/fleet_units?id=eq.${encodeURIComponent(id)}&select=id,code,vehicle_type,model,source_kv_key`, {
    headers: {
...
const id = `smoke-fleet-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
const code = `SMK-${Date.now()}`;
const unit = {
  id,
  code,
  vehicleType: "staging smoke vehicle",
  model: "SMOKE",
  supplier: "staging",
  status: "active",
  createdAt: Date.now(),
...
try {
  const upsertResponse = await fetch(`${publicUrl}/api/fleet`, {
    method: "POST",
    headers: {
  ...
  if (!upsertResponse.ok || !upsertData?.ok) throw new Error(upsertData?.error || `fleet_upsert_${upsertResponse.status}`);

  const afterUpsert = await fleetRows({ root: supabaseUrl, serviceRoleKey, id });
  if (afterUpsert.length !== 1) throw new Error(`fleet_upsert_row_count:${afterUpsert.length}`);
  if (afterUpsert[0].code !== code) throw new Error("fleet_upsert_code_mismatch");

  const listResponse = await fetch(`${publicUrl}/api/fleet`, {
    headers: { authorization: `Bearer ${accessToken}` }
  });
  const listData = await readJson(listResponse);
  if (!listResponse.ok || !listData?.ok) throw new Error(listData?.error || `fleet_list_${listResponse.status}`);
  if (!listData?.units?.some((item) => item.id === id && item.code === code)) throw new Error("fleet_list_missing_smoke_unit");

  const deleteResponse = await fetch(`${publicUrl}/api/fleet?id=${encodeURIComponent(id)}`, {
    method: "DELETE",
    headers: { authorization: `Bearer ${accessToken}` }
  });
  const deleteData = await readJson(deleteResponse);
  if (!deleteResponse.ok || !deleteData?.ok) throw new Error(deleteData?.error || `fleet_delete_${deleteResponse.status}`);

  const afterDelete = await fleetRows({ root: supabaseUrl, serviceRoleKey, id });
  if (afterDelete.length !== 0) throw new Error(`fleet_delete_row_count:${afterDelete.length}`);
  ...
  try {
    await fetch(`${publicUrl}/api/fleet?id=${encodeURIComponent(id)}`, {
      method: "DELETE",
      headers: { authorization: `Bearer ${accessToken}` }
    });
  } catch {}
