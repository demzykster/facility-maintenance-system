...
const RESOURCES = Object.freeze([
  {
    ...
    singular: "meeting",
    plural: "meetings",
    table: "maintenance_meetings",
    id: `smoke-meeting-${suffix}`,
    record: { id: `smoke-meeting-${suffix}`, title: `Smoke Meeting ${suffix}`, status: "planned", at: now, createdAt: now }
  },
  {
    resource: "tasks",
    singular: "task",
    ...
    id: `smoke-task-${suffix}`,
    record: { id: `smoke-task-${suffix}`, title: `Smoke Task ${suffix}`, status: "open", source: "smoke", createdAt: now }
  }
]);
...
async function upsert({ publicUrl, accessToken, config }) {
  const response = await fetch(`${publicUrl}/api/work`, {
    method: "POST",
    headers: { authorization: `Bearer ${accessToken}`, "content-type": "application/json" },
    body: JSON.stringify({ resource: config.resource, [config.singular]: config.record })
  });
  const data = await readJson(response);
  if (!response.ok || !data?.ok) throw new Error(data?.error || `work_${config.resource}_upsert_${response.status}`);
}

async function list({ publicUrl, accessToken, config }) {
  const response = await fetch(`${publicUrl}/api/work?resource=${encodeURIComponent(config.resource)}`, {
    headers: { authorization: `Bearer ${accessToken}` }
  });
  const data = await readJson(response);
  if (!response.ok || !data?.ok) throw new Error(data?.error || `work_${config.resource}_list_${response.status}`);
  return Array.isArray(data?.[config.plural]) ? data[config.plural] : [];
}

async function remove({ publicUrl, accessToken, config }) {
  const response = await fetch(`${publicUrl}/api/work?resource=${encodeURIComponent(config.resource)}&id=${encodeURIComponent(config.id)}`, {
    method: "DELETE",
    headers: { authorization: `Bearer ${accessToken}` }
  });
  const data = await readJson(response);
  if (!response.ok || !data?.ok) throw new Error(data?.error || `work_${config.resource}_delete_${response.status}`);
}
...
try {
  for (const config of RESOURCES) {
    ...
    if (afterUpsert.length !== 1) throw new Error(`work_${config.resource}_upsert_row_count:${afterUpsert.length}`);
    const listed = await list({ publicUrl, accessToken, config });
    if (!listed.some((record) => record.id === config.id)) throw new Error(`work_${config.resource}_list_missing_smoke_record`);
  ...
  for (const config of [...RESOURCES].reverse()) {
    ...
    if (afterDelete.length !== 0) throw new Error(`work_${config.resource}_delete_row_count:${afterDelete.length}`);
    results[config.resource].deleted = true;
  }

  console.log(JSON.stringify({ ok: true, appUrl: publicUrl, work: results }, null, 2));
} catch (error) {
  for (const config of [...RESOURCES].reverse()) {
    try {
      await fetch(`${publicUrl}/api/work?resource=${encodeURIComponent(config.resource)}&id=${encodeURIComponent(config.id)}`, {
        method: "DELETE",
        headers: { authorization: `Bearer ${accessToken}` }
      });
    } catch {}
