...
The gate also runs controlled normalized API smokes for `/api/tickets`, `/api/fleet`, `/api/pm`, `/api/cleaning/zones`, and `/api/cleaning/rounds`. Each smoke creates one temporary record, verifies it in its Supabase table, deletes it through the same API route, and verifies cleanup. Before the business smokes, the gate reconciles legacy KV records for tickets, fleet, periodic maintenance, cleaning zones, and cleaning rounds into their normalized tables. Cleaning zones use normalized API authority in production/API mode with `czone:` as a compatibility mirror; cleaning rounds are mirrored into `public.cleaning_rounds` for staging safety, but UI/runtime authority still remains on `cround:` until a later slice. Cleaning complaints and absences remain on the KV bridge until later slices.
