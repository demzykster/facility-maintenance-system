...
- Ticket photos are managed through `/api/files` plus `file_metadata`; generic production/API KV writes for `photo:*` are retired no-ops while demo/local legacy photo storage remains available outside production/API mode.
...
- Settings records are managed through `/api/settings/records`; normalized `locations` and `app_issue_reports` are the production/API-mode authority, production/API app issue saves and automatic app issue reports no longer create new `appIssue:*` KV mirrors, and generic production/API KV writes for `location:*` and `appIssue:*` are retired no-ops.
