...
- `npm run staging:users:reconcile-report` compares legacy `user:` KV records with `public.app_users` and can backfill safe legacy-only rows with `-- --apply` without deleting KV mirrors. `npm run staging:users:retire-kv-mirrors` is the guarded dry-run/apply command for deleting old `user:*` mirrors only when every legacy row is matched to `app_users`. On 2026-07-10 staging apply inserted 9 remaining users into `app_users`; the follow-up report showed `legacyUsers=12`, `appUsers=19`, `matched=12`, `legacyOnly=0`, `ambiguous=0`, `parseErrors=0`, `proposedBackfill=0`, and `skippedBackfill=0`.
