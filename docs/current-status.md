...
- Owner-reported iPhone startup slowness on 2026-07-10 traced to authenticated startup data hydration, not the unauthenticated shell. The active fix is to skip the old shared KV startup sweep for domains with normalized API authority, preserve KV fallback only for API failures, and load independent normalized APIs in parallel.
