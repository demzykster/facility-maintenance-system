...
- `npm run staging:users:reconcile-report` is the read-only dry-run for comparing legacy `user:` KV records with `public.app_users` before any user migration or deletion decision.
