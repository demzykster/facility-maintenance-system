...
- `npm run staging:kv:residuals` is the read-only residual KV classifier. On 2026-07-10, after the owner approved removing the inactive controls/template leftovers, staging deleted `controlProgram:`, `controlAssignment:`, `controlRun:`, `controlFinding:`, and `itpl:` KV records. The follow-up report classified 219 shared KV records as normalized compatibility mirrors, 0 as transient operational records, 0 as deferred/orphan candidates, and 0 as unknown.
