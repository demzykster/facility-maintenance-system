...
- `npm run staging:kv:residuals` is the read-only residual KV classifier. On 2026-07-10 it classified 219 shared KV records as normalized compatibility mirrors, 0 as transient operational records, 5 as deferred/orphan candidates, and 0 as unknown. The only deferred/orphan candidates are the inactive/undriven `controlProgram:`, `controlAssignment:`, `controlRun:`, `controlFinding:`, and `itpl:` records. Do not normalize or delete those without a product decision for the controls/template domain.
