...
- `npm test` is available. As of 2026-07-10 on the session-route consolidation branch, it runs 169 test files / 828 tests.
...
- Session API entrypoints keep the existing `/api/session/*` URLs but are served by one dynamic route file, leaving the Vercel API route budget at 18/24.
