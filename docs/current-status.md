...
- `npm test` is available. As of 2026-07-10 on the diagnostics-route consolidation branch, it runs 170 test files / 831 tests.
...
- Production bundle is still above Vite's default 500 kB chunk warning. On 2026-07-10, `npm run build` produced the main app chunk at 2,204.42 kB raw / 579.39 kB gzip. This is expected for the current monolith and is not a blocker, but it remains a real size warning.
...
- Session API entrypoints keep the existing `/api/session/*` URLs but are served by one dynamic route file. Client/system diagnostics keep the existing `/api/client-errors` and `/api/system-errors` URLs but are served by one dynamic diagnostics route, leaving the Vercel API route budget at 17/24.
