...
- Locations and app issue reports are normalized-authority in production/API mode through `/api/settings/records`; production/API app issue saves no longer create new `appIssue:*` KV mirrors, while existing `location:*` and `appIssue:*` KV records remain compatibility mirrors until retirement.
- Technician/user presence is normalized-authority in production/API mode through `/api/presence`; production/API saves no longer create new `presence:*` KV mirrors, while existing `presence:*` KV records remain compatibility mirrors until retirement.
