...
- Cleaning zones, rounds, complaints, and worker absences are normalized-authority in production/API mode through `/api/cleaning/records`; public QR zone listing and public complaint zone lookup also read normalized cleaning zones before falling back to `czone:` KV mirrors; their legacy KV prefixes remain compatibility mirrors.
