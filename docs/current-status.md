...
- Cleaning zones, rounds, complaints, and worker absences are normalized-authority in production/API mode through `/api/cleaning/records`; production/API saves no longer create new `czone:*`, `cround:*`, `ccomplaint:*`, or `cabsence:*` KV mirrors. Public QR zone listing and public complaint zone lookup also read normalized cleaning zones before falling back to legacy `czone:` KV mirrors when present.
