...
- Production bundle is still above Vite's default 500 kB raw/minified chunk warning. On 2026-07-13, `npm run build` produced the main app chunk at 930.52 kB raw / 233.17 kB gzip, plus lazy chunks for Excel/CSV import/export, QR scanning/printing, notifications, AI, BI overview/heatmap, fleet/transport and periodic maintenance, settings/team management, suppliers, activity log, PPE dashboards/orders, management tasks/meetings, admin ticket lists, app issue settings, and ticket detail. This is improved from the previous 2,208.37 kB raw / 580.14 kB gzip main chunk, but it remains a real raw-size warning.
- `npm audit` is currently clean: 0 vulnerabilities on 2026-07-09.
- The old `xlsx` package dependency has been removed from `package.json`. Excel workbook helpers now live in `src/xlsxWorkbookModel.js`; the heavier `write-excel-file` export path is lazy-loaded through `src/xlsxExportAdapter.js`. QR printing/scanning lazy-loads `qrcode` and `jsqr`; Excel/CSV import lazy-loads `read-excel-file` and `papaparse`.
- Owner-reported iPhone startup slowness on 2026-07-10 traced to authenticated startup data hydration, not the unauthenticated shell. The deployed fix skips the old shared KV startup sweep for domains with normalized API authority, preserves KV fallback only for API failures, and loads independent normalized APIs in parallel. A post-deploy live-style authenticated hydration sample measured about 0.96s wall time after session restore.
- The follow-up authenticated refresh throttle reduces full background data polling from the old 15-second loop to a calmer guarded schedule, prevents overlapping `reloadAll()` runs, and still refreshes immediately when the app returns to the foreground.
- Staging load-test smoke is available through `npm run staging:load:test -- --profile smoke`. On 2026-07-10 the default API p95 threshold was calibrated to 1500ms because the smoke samples only eight API operations, making p95 effectively the max sample. The max threshold remains 3000ms; the calibrated smoke passed with p95/max 928ms and cleanup remaining rows at 0. On 2026-07-11 the heavier `pilot` profile passed on the live Vercel/Supabase staging environment with 1000 tickets, 1000 tasks, 100 meetings, 250 fleet units, and 500 cleaning complaints; API p95/max was 1126ms and cleanup left 0 temporary business rows.
...
- Production/API ticket photos use `/api/files` plus `file_metadata`; generic production/API KV writes for legacy `photo:*` records are retired no-ops while demo/local legacy photo storage remains available outside production/API mode.
...
- Periodic maintenance is normalized-authority in production/API mode through `/api/pm`; production/API saves/batch saves/deletes no longer create new `pm:*` KV mirrors.
...
- Maintenance tasks and meetings are normalized-authority in production/API mode through `/api/work`; production/API saves/deletes no longer create new `mtask:*` or `mmeet:*` KV mirrors.
...
- Locations and app issue reports are normalized-authority in production/API mode through `/api/settings/records`; production/API app issue saves and automatic app issue reports no longer create new `appIssue:*` KV mirrors, generic production/API KV writes for `location:*` and `appIssue:*` are retired no-ops, and staging has retired all matched `appIssue:*` KV mirrors.
...
- R10 production data-core cleanup is complete for the currently mapped v1 business domains: normalized tables/server routes exist, production/API writes no longer recreate KV mirrors, the shared KV residual report is empty, and the generic production/API KV write bridge is retired for all allowed shared prefixes. On 2026-07-11 the Vercel production environment variable contract included all required staging gate names, including `CMMS_DATA_AUTHORITY=normalized`. Final production labeling still needs owner acceptance plus the independent architecture/security review.
