...
| Cleaning round reminder (`תזכורת לפני סבב ניקיון`) | `הגדרות` -> `כללי` | `בקרת ניקיון` -> settings | Live setting. It belongs with cleaning rounds/windows, but do not move it until the cleaning module has a stable settings surface. |
| Global notification type toggles (`סוגי התראות`) | `הגדרות` -> `כללי` compact `מדיניות התראות מערכת` block | Dedicated notification policy screen if needed | This is a system-wide notification policy, not personal filtering. Keep distinct from the notification panel's local user filters. Do not make it visually dominant because disabling a kind affects everyone. |
