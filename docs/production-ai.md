...
- Live Vercel status checked on 2026-07-13: production currently has no AI env configured, so authenticated `/api/ai/status` correctly reports `mode: "disabled"`, `serverReady: false`, and `ai_server_disabled`. The code path is ready for server AI, but live AI cannot be connected until production Vercel env includes `CMMS_AI_MODE=server`, `CMMS_AI_PROVIDER`, and a matching server-side provider key (`OPENAI_API_KEY` or `ANTHROPIC_API_KEY`).
