...
- expand the action surface one operation at a time, always through an existing normal server operation with validation, authorization, and audit;
- add guided completion UI for incomplete proposals, especially transport tickets that require an exact fleet unit and downtime type before creation;
