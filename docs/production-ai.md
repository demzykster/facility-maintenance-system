...
- expand the action surface one operation at a time, always through an existing normal server operation with validation, authorization, and audit. The current action foundations are `ticket.create` and a constrained `ticket.update`; next slices should connect deterministic proposal generation for concrete update scenarios instead of trusting provider free text;
