...
- Dedicated screen-level AI entry points now exist in the BI ticket heatmap and in ticket detail. `src/aiAssistEntryPointModel.js` builds human-reviewable heatmap and ticket questions, `src/BIHeatmapPanel.jsx` exposes the heatmap entry point, `TicketDetail` exposes the ticket entry point, and `src/AIPanel.jsx` opens with the suggested question prefilled. The assistant still waits for the human to send the question and remains read-only.
...
- repeat the same dedicated-entry pattern for fleet/PPE/cleaning risk panels and supplier queues where a user naturally wants an explanation or next-action summary;
