...
- Cleaning zones staging slice: `public.cleaning_zones` and `/api/cleaning/zones` are live for server operations. The staging gate reconciles protected `czone:*` KV records into `public.cleaning_zones` through the normalized route and runs a controlled `/api/cleaning/zones` smoke, while production/API UI runtime authority remains on the KV bridge until a later authority slice.
