...
- Cleaning rounds API seam: `public.cleaning_rounds` and `/api/cleaning/rounds` are available as the next server-operation seam. Runtime authority, staging reconciliation for `cround:*`, and staging API smoke are intentionally left for later narrow slices.
