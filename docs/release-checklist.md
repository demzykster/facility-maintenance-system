...
- Cleaning zones authority slice: `public.cleaning_zones` is live through `/api/cleaning/records?resource=zones`. In production/API mode, cleaning zone reads, saves, and deletes use the normalized API first, and public QR zone listing plus public complaint zone lookup read normalized cleaning zones before falling back to `czone:*` KV. `czone:*` KV is only a compatibility mirror. The staging gate reconciles protected `czone:*` KV records into `public.cleaning_zones` and runs a controlled cleaning zones smoke.
- Cleaning rounds authority slice: `public.cleaning_rounds` is live through `/api/cleaning/records?resource=rounds`. In production/API mode, cleaning round reads and saves use the normalized API first, while `cround:*` KV is only a compatibility mirror. The staging gate reconciles protected `cround:*` KV records into `public.cleaning_rounds` and runs a controlled cleaning rounds smoke.
...
- PPE authority slice: `public.ppe_items`, `public.ppe_norms`, `public.ppe_movements`, `public.ppe_requests`, and `public.ppe_orders` are live through one `/api/ppe` route. In production/API mode, PPE reads, saves, requests, catalog/norm changes, movements, and orders use the normalized API first, while `ppe:*`, `ppeitem:*`, `ppenorm:*`, `ppereq:*`, and `ppeorder:*` KV records remain compatibility mirrors. The staging gate reconciles PPE KV records into the normalized PPE tables and runs a controlled `/api/ppe` smoke.
...
- Settings-records authority slice: `public.locations` and `public.app_issue_reports` are served through one grouped `/api/settings/records` route. In production/API mode, locations and app issue reports read/write through the normalized API first while `location:*` and `appIssue:*` KV records remain compatibility mirrors; the staging gate reconciles those KV records and runs a controlled settings-records API smoke.
- App-config authority slice: `public.app_config` is served through `/api/settings/config`. In production/API mode, `config:v1` reads/writes go through the normalized API first while `config:v1` remains a compatibility mirror; the staging gate reconciles the old KV record and runs a controlled config API smoke. Adding this route leaves the Vercel API route budget at 19/24.
...
- Presence authority slice: `public.technician_presence` and `/api/presence` are live. In production/API mode, technician/user presence reads and writes use the normalized API first, and production/API saves no longer create new `presence:*` KV mirrors. Existing `presence:*` KV records remain compatibility mirrors until retirement. The staging gate reconciles `presence:*` into `public.technician_presence` and runs a controlled `/api/presence` smoke. Adding this route leaves the Vercel API route budget at 18/24.
- Push-subscriptions authority slice: `public.push_subscriptions` is the normalized table behind `/api/push`. In production/API mode, phone push subscribe/unsubscribe uses normalized storage first, while `pushSubscriptions:v1` remains a compatibility mirror. The staging gate reconciles the old KV JSON list into `public.push_subscriptions` and runs a controlled subscribe/unsubscribe smoke.
