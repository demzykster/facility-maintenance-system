...
- Ticket authority slice: in production/API mode, the app reads tickets from `/api/tickets`, writes/deletes through the normalized tickets API first, and no longer creates new `ticket:*` KV mirrors. Demo/local mode still uses the existing KV path.
