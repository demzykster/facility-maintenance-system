...
- Cleaning rounds staging slice: `public.cleaning_rounds` and `/api/cleaning/rounds` are available as the next server-operation seam. The staging gate reconciles protected `cround:*` KV records into `public.cleaning_rounds` through the normalized route and runs a controlled `/api/cleaning/rounds` smoke, while UI/runtime authority remains on the KV bridge until a later authority slice.
