...
- Cleaning zones authority slice: `public.cleaning_zones` and `/api/cleaning/zones` are live. In production/API mode, cleaning zone reads, saves, and deletes use the normalized API first, while `czone:*` KV is only a compatibility mirror. The staging gate reconciles protected `czone:*` KV records into `public.cleaning_zones` and runs a controlled `/api/cleaning/zones` smoke. Cleaning rounds, complaints, and absences still use the KV bridge until later cleaning slices.
- Cleaning rounds authority slice: `public.cleaning_rounds` and `/api/cleaning/rounds` are live. In production/API mode, cleaning round reads and saves use the normalized API first, while `cround:*` KV is only a compatibility mirror. The staging gate reconciles protected `cround:*` KV records into `public.cleaning_rounds` and runs a controlled `/api/cleaning/rounds` smoke. Cleaning complaints and absences still use the KV bridge until later cleaning slices.
...
- First cleaning server-operation slice: `/api/cleaning/zones` can list/upsert/delete normalized `public.cleaning_zones` rows with the same settings-management write boundary as `czone:`. The production UI still reads/writes the KV bridge until a later authority slice.
