...
- `Add Google Gemini AI provider support` (current local slice)
  ...
  - `server/ai/providerClient.js` now supports the Gemini `generateContent` request shape with server-only key headers and safe text extraction. `server/ai/statusHandler.js` passes the Google key only server-side for admin provider checks.
  ...
  - This still does not use Vercel AI SDK or LangChain. The documented architecture decision is to keep the deterministic human-confirmed action layer as the safety boundary, then introduce Vercel AI SDK Core behind `server/ai/providerClient.js` as an internal implementation detail in a later slice.
  - Tests cover provider aliases/readiness, Gemini request construction, missing-key handling, status checks, provider-neutral UI labels, and `/api/ai/assist` provider calls without returning secrets.
