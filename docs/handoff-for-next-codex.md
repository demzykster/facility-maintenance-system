...
- `Migrate AI provider boundary to Vercel AI SDK Core` (current local slice)
  ...
  - `src/aiProviderModel.js` now exposes explicit supported model options alongside provider options. Current settings options are `claude-sonnet-4-20250514`, `gemini-3.5-flash`, `gemini-3.1-flash-lite`, and `gpt-5.2`.
...
- `Add Google Gemini AI provider support` (current local slice)
  - `src/aiProviderModel.js` now recognizes `google` / `gemini`, exposes `Google / Gemini`, defaults that provider to `gemini-3.5-flash`, and checks `GOOGLE_GENERATIVE_AI_API_KEY` / `GOOGLE_API_KEY` server env readiness.
