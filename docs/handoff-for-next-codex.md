...
- Live AI connection note: on 2026-07-13, authenticated `/api/ai/status` on Vercel initially reported `mode: "disabled"`, `serverReady: false`, and `errors: ["ai_server_disabled"]` because no production AI env existed. Production env was then configured with `CMMS_AI_MODE=server`, `CMMS_AI_PROVIDER=openai`, `CMMS_AI_MODEL`, and a sensitive `OPENAI_API_KEY`; do not store or print the key in repo/docs. The first post-env provider check showed the live server had the env (`serverReady=true`) but failed because our status check sent `max_output_tokens=8`, below OpenAI's minimum. The current code fixes that by using/clamping to 16. After the fix deployed, `/api/ai/status?check=1` reached OpenAI but the configured account/key returned a quota/billing failure. The UI now normalizes that as `ai_provider_quota_exceeded` instead of a generic network/offline error. Live AI still needs usable OpenAI quota/billing or a different working server-side key before model answers can succeed.
