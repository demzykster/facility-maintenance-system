...
- Live AI connection note: on 2026-07-13, authenticated `/api/ai/status` on Vercel initially reported `mode: "disabled"`, `serverReady: false`, and `errors: ["ai_server_disabled"]` because no production AI env existed. Production env was then configured with `CMMS_AI_MODE=server`, `CMMS_AI_PROVIDER=openai`, `CMMS_AI_MODEL`, and a sensitive `OPENAI_API_KEY`; do not store or print the key in repo/docs. The first post-env provider check showed the live server had the env (`serverReady=true`) but failed because our status check sent `max_output_tokens=8`, below OpenAI's minimum. The current code fixes that by using/clamping provider checks to 16. After the fix deployed, `/api/ai/status?check=1` reached OpenAI but the configured account/key returned a quota/billing failure. The UI now normalizes that as `ai_provider_quota_exceeded` instead of a generic network/offline error. The server provider boundary also supports Google Gemini through `GOOGLE_GENERATIVE_AI_API_KEY`; live AI can switch to `CMMS_AI_PROVIDER=gemini` / `google` without browser secrets or UI rewrites after redeploy. Live follow-up testing showed `gemini-2.5-flash` is unavailable for the configured key, `gemini-1.5-flash` is unavailable, and `gemini-2.0-flash` reaches the provider but returns quota/billing failure. Keep production on `gemini-2.0-flash` until the Google project quota/billing is fixed or another working provider key is supplied. Provider transport now uses Vercel AI SDK Core behind `server/ai/providerClient.js`; the optional `callAiProviderObject()` structured-output seam can produce sanitized non-writing plans when `/api/ai/assist` is called with `includeProviderPlan` / `structuredPlan`, but the human-confirmed CMMS action layer remains the write-safety boundary.
...
- `Add non-writing AI provider plan seam` (current local slice)
...
- `Migrate AI provider boundary to Vercel AI SDK Core` (current local slice)
  ...
  - `src/aiProviderModel.js` now exposes explicit supported model options alongside provider options. Current settings options are `claude-sonnet-4-20250514`, `gemini-2.0-flash`, and `gpt-5.2`.
  - `src/AISettingsCard.jsx` now renders model selection from those supported options while preserving any pre-existing custom model value as a visible custom option.
  - This is not direct AI database write access. Provider-native output still produces assistant text/proposals only; every CMMS mutation remains a human-confirmed app action through the existing authenticated operation path.
- `Add Google Gemini AI provider support` (current local slice)
  - `src/aiProviderModel.js` now recognizes `google` / `gemini`, exposes `Google / Gemini`, defaults that provider to `gemini-2.0-flash`, and checks `GOOGLE_GENERATIVE_AI_API_KEY` / `GOOGLE_API_KEY` server env readiness.
  - `server/ai/statusHandler.js` passes the Google key only server-side for admin provider checks.
  - `src/AISettingsCard.jsx` and `src/AIPanel.jsx` use provider-neutral quota wording so OpenAI/Gemini/Anthropic failures do not show as the wrong vendor.
  - Tests cover provider aliases/readiness, Gemini SDK model selection, missing-key handling, status checks, provider-neutral UI labels, and `/api/ai/assist` provider calls without returning secrets.
- `Prefill AI transport ticket fleet unit` (current local slice)
  ...
  - If the same new-ticket text explicitly says no replacement / critical downtime, replacement available, or can continue working / minor downtime, the draft can also pre-fill `downtimeType` as `critical`, `has_replacement`, or `minor`.
  - Hidden, ambiguous, or absent fleet codes are not guessed. Vague fault text does not infer `downtimeType`. The draft remains `writesData: false` and still requires human confirmation or normal form submission.
  - `tests/aiAssistActionModel.test.js` covers unique/ambiguous fleet matching and explicit downtime matching at the model seam; `tests/aiAssistHandler.test.js` covers the same behavior after server role filtering.
...
- `Fix OpenAI AI status token floor` (current local slice)
  - `server/ai/statusHandler.js` now asks the provider check for 16 output tokens instead of 8.
  - `server/ai/providerClient.js` clamps OpenAI Responses API calls to at least 16 output tokens so future tiny checks do not fail provider validation.
  - `tests/aiStatusHandler.test.js` and `tests/aiProviderClient.test.js` cover the status route and provider adapter seams.
- `Normalize AI provider quota failures` (current local slice)
  - `server/ai/statusHandler.js` and `server/ai/assistHandler.js` normalize provider quota/billing, auth, model, and rate-limit failures into stable `ai_provider_*` codes.
  - `src/AISettingsCard.jsx`, `src/AIPanel.jsx`, and `callAIAssistant()` now surface quota/billing failures as an operator/user-facing AI provider problem instead of a generic offline/network message.
  - Tests cover settings labels, panel messages, `/api/ai/status?check=1`, and `/api/ai/assist` failure responses.
...
- `Add human-confirmed AI meeting time updates` (current local slice)
  ...
  - The action remains `writesData: false`, requires human confirmation, changes only the allow-listed `at` field, and targets the existing `/api/work` contract with `resource: "meetings"` / `bodyField: "meeting"`.
  - `src/aiAssistActionExecutionModel.js` prepares the confirmed update for the existing `saveMeeting` path, preserves the existing meeting id, ignores non-allow-listed patch fields, records a diff, and appends an `ai_confirmed_meeting_update` log entry.
  - `src/AIPanel.jsx` renders meeting update before/after rows, and `LazyAIPanel` executes the confirmed update through `saveMeeting`, not through a parallel AI write path.
  - This slice intentionally does not infer participants, rooms, recurring rules, deletes, or ambiguous meeting targets. Those remain separate future safety slices.
