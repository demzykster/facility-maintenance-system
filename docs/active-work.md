...
- Active branch: `codex/maintenance-targets-imported-vehicle-kind`.
- In progress: continue the TO/periodic-maintenance hardening without starting the monolith split. This branch verifies that imported fleet units keep `סוג כלי` and `דגם` separate when matching owner-defined maintenance regulations.
- Current branch change: make periodic-maintenance targeting read imported `vehicleKind` as the vehicle type and keep `model` as the model code, with tests proving TO rules match by imported vehicle type without catalog fallback.
- Latest completed work: PR #527 fixed fleet catalog/import validation so imported fleet units keep `סוג כלי` and `דגם` as separate catalog dimensions. Main is currently at `63bc69b`.
- Current branch validation target: fleet maintenance/catalog/import tests, full unit tests, `release:check`, `build`, and `git diff --check`.
...
- Latest completed work: added a repeatable `staging:smoke:fleet-ui` check to compare fleet counts across Supabase, `/api/kv`, and the live fleet UI. The check is now part of `staging:gate` so the exact class of issue where records exist server-side but a fresh browser shows an empty fleet screen is covered before release.
- Validation for the fleet UI smoke gate: `npm run staging:smoke:fleet-ui` passed with Supabase/API/UI all reporting 128 fleet records; full `npm run staging:gate` passed on deployed commit `06bdbfd`, including the new fleet UI count check; `npm test -- --run`, `npm run release:check`, and `npm run build` passed.
- Current safety work: continue from the remaining data integrity / permissions / UX stabilization items. Do not start the larger TO/checklist redesign until the owner workflow is verified and split into small PRs.
- Latest completed work: optimized atomic `/api/kv` batch writes so fleet Excel imports using server-side `setMany` no longer prefetch every record's previous value just for rollback/audit. Ticket status batches still prefetch previous ticket values so status-transition audit history remains intact.
- Validation for the atomic KV batch optimization: targeted KV/import/storage tests passed, full `npm test -- --run` passed, `npm run release:check` passed, `git diff --check` passed, `npm run build` passed, and the supplied `רישיונות` workbook preview still reads 128 ready rows with 13 vehicle-type catalog additions while keeping `סוג כלי` separate from `דגם`.
- Latest completed work: refined the periodic-maintenance settings UX so owner-defined TO programs are labeled as `תכניות טיפול תקופתי`, keep their own checklist items, show affected fleet counts, and offer bulk target selection for all vehicle types/models. The legacy per-type `pmFreq` field is now collapsed as backward compatibility for old manual entries.
- Validation for the maintenance-settings UX refinement: targeted fleet maintenance policy tests passed, full `npm test -- --run` passed, `npm run release:check` passed, `git diff --check` passed, `npm run build` passed, and local Playwright smoke confirmed the app renders without console errors or horizontal overflow.
...
- Validation for PR #506: targeted fleet maintenance policy tests passed, full `npm test -- --run` passed, `npm run release:check` passed, `git diff --check` passed, `npm run build` passed, and local Playwright smoke confirmed `כלי שינוע → לוח טיפולים` opens the bulk regulation scheduler with the separated periodic-maintenance checklist note, no console errors, and no horizontal overflow.
- Latest completed work: PR #505 connected owner-defined maintenance regulations from `כלי שינוע → הגדרות` to the periodic maintenance schedule. New schedule entries can select a matching rule, store its name/month interval, calculate the next due date by calendar months, and show the rule name in lists/history/exports. Legacy `pmFreq` entries remain readable as a fallback.
- Validation for PR #505: targeted fleet maintenance policy tests passed, full `npm test -- --run` passed, `npm run release:check` passed, `git diff --check` passed, `npm run build` passed, local Playwright smoke confirmed `כלי שינוע → לוח טיפולים` plus the periodic-maintenance scheduling form render without a red save-failure banner, console errors, or horizontal overflow, and GitHub Actions/Vercel passed before merge.
- Previous completed work: PR #504 added the first UI settings surface for owner-defined maintenance regulations in `כלי שינוע → הגדרות`. This stores rule names, month intervals, and type/model targets while keeping the old periodic calendar wiring intact for the next PR.
...
- Current facts to preserve:
  ...
  - PWA login install prompt copy/desktop follow-up: the login prompt now frames install as device notifications, uses a desktop/browser install icon when `beforeinstallprompt` is available, keeps iPhone as Share -> Add to Home Screen instructions, and uses an icon-only install button with accessible label/title.
  - KV collection loading should prefer one `/api/kv?prefix=...&includeValues=1` call per collection over key-list plus one GET per key, while keeping the old list/get path as fallback.
  ...
  - Fleet Excel import save failures should stay contextual: preview remains read-only, and failed catalog/fleet writes during the final import stop the import with an inline modal error instead of continuing as if all rows were saved or showing only a global red save-failure banner.
