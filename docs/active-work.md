...
- Active branch: `none`.
- Current branch: `main`.
- Last completed work: supplier management now keeps technicians visibly linked to suppliers. Supplier cards can add new suppliers reliably, supplier detail has a `טכנאים` tab, technician profiles keep supplier assignment for every technician scope, and the existing transport-ticket supplier visibility contract is covered by a regression test.
- Current work: none.
...
- Fleet internal numbers require the Supabase migration `20260712103000_fleet_units_internal_no.sql` before production/staging normalized `fleet_units` rows can store direct `internal_no` values. Existing fleet records remain valid; fill internal numbers manually through the fleet edit form or by updating the normalized column/payload through controlled admin tooling.
- Supplier-linked technicians are not a new table yet: the current contract uses the existing `app_users.supplier` / session `supplier` profile field, fleet `supplier`, and transport ticket visibility. If this grows into supplier-side dispatch workflows, extend that contract deliberately instead of adding a parallel assignment path.
