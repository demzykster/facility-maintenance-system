...
- Active branch: `codex/bulk-maintenance-rule-schedules`.
...
- Current PR scope: add bulk generation/update of periodic-maintenance schedule entries from owner-defined maintenance regulations. The owner can generate PM entries for all matching fleet units without selecting every vehicle one by one. Existing `vehicle + regulation` entries are updated without duplication; new entries receive the chosen first due date.
- Current correction: periodic-maintenance regulations must not reuse `בקרת כלים` checklist templates. Those are separate concepts. Current PR removes that link from new regulation/schedule creation and leaves periodic-maintenance checklist design for a separate future feature.
