...
- Active branch: `none`.
- Current branch: `main`.
- Last completed work: fleet units now support a separate internal number (`internalNo` in app records, `internal_no` in Supabase) alongside the existing code, chassis number, and license number. Transport search, fleet forms/cards, driver coverage/actions, ticket opening, PM follow-up tickets, exports, notifications, and optional fleet-license Excel import now preserve or display the internal number with safe fallback to the old `code`.
- Current work: none.
