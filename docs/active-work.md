...
- Active branch: `codex/separate-pm-checklists`.
...
- Current PR scope: make periodic-maintenance checklist items their own rule-level data, separate from `בקרת כלים` inspection questionnaires. PM rules can define their own treatment checklist, generated/manual PM entries carry that checklist snapshot, and PM completion history records checked items.
- Current correction: do not use, select, or borrow `בקרת כלים` checklist templates for `רגולציות טיפול תקופתי`. Those are separate business objects.
- Newly recorded pending issue, not in this PR: vehicle type catalog persistence is inconsistent. After deleting fleet units, deleted `סוגי כלי שינוע` entries can reappear after navigation, and Excel import catalog additions do not fully reconcile the type catalog. Investigate next as a separate catalog-persistence/import PR.
- Validation so far: targeted fleet maintenance policy tests passed, full `npm test -- --run` passed, `npm run release:check` passed, `git diff --check` passed, `npm run build` passed, and local Playwright smoke confirmed the app renders without console/page errors.
